...
class PostPage(Handler):
    def get(self, post_id):
        ...
        self.write('<h1>' + post.title + '</h1><br>' + post.body + '<br><a href="../blog">Main</a>')
...
class BlogJson(JsonHandler):
    def get(self):
        ...
        for entry in entries:
            res.append({'content':entry.body, 'subject':entry.title})
...
class PostJson(JsonHandler):
    def get(self, post_id):
        post = Entry.get_by_id(int(post_id), None)
        self.write(json.dumps({'content':post.body, 'subject':post.title}))
...
application = webapp2.WSGIApplication([('/', MainPage),
                                       ...
                                       # the parentheses mean that what's inside is passed to get() as an arg
                                       ('/(\d+)', PostPage),
                                       ('/(\d+).json', PostJson),
